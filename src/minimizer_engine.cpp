...
std::vector<biosoup::Overlap> MinimizerEngine::Map(
  ...
  for (const auto& it : sketch) {
    ...
    for (std::uint32_t j = 0; j < n; ++j, ++jt) {
      if (avoid_equal && sequence->id == jt->id()) {
        continue;
      }
      if (avoid_symmetric && sequence->id > jt->id()) {
        continue;
      }

      std::uint64_t rhs_id = jt->id();
      std::uint64_t strand_ = it.strand() == jt->strand();
      std::uint64_t lhs_pos = it.position();
      std::uint16_t lhs_span = it.span();
      std::uint64_t rhs_pos = jt->position();
      std::uint16_t rhs_span = jt->span();
...
std::vector<MinimizerEngine::Kmer> MinimizerEngine::Minimize(
  ...
  auto window_add = [&] (std::uint64_t description, std::uint64_t origin) -> void {  // NOLINT
  ...
  std::deque<std::pair<std::uint64_t, std::uint64_t>> compressed_kmer;
  std::uint64_t span = 0;
  ...
  for (std::uint32_t i = 0, j = 0; i < sequence->inflated_len; ++i) {
    ...
    if (hpc) {
      if (!compressed_kmer.empty() && compressed_kmer.back().first == c) {
        ++compressed_kmer.back().second;
        ++span;
        continue;
      } else {
        compressed_kmer.emplace_back(c, 0);
      }
    }

    minimizer = ((minimizer << 2) | c) & mask;
    reverse_minimizer = (reverse_minimizer >> 2) | ((c ^ 3) << shift);
    if (j >= k_ - 1U && span < 257ULL) {
      std::uint64_t origin =
          ((i - (k_ - 1U) - span) << 33) |
          ((j - (k_ - 1U)) << 1);

      if (minimizer < reverse_minimizer) {
        window_add((hash(minimizer) << 8) | span, origin);
      } else if (minimizer > reverse_minimizer) {
        origin |= 1ULL << 32;
        window_add((hash(reverse_minimizer) << 8) | span, origin);
      }
    }
    if (j >= (k_ - 1U) + (w_ - 1U)) {
      ...
      window_update(j - (k_ - 1U) - (w_ - 1U) + 1);
    }

    if (hpc) {
      if (compressed_kmer.size() == k_) {
        span -= compressed_kmer.front().second;
        compressed_kmer.pop_front();
      }
    }
